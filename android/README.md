...
- 浮窗调节面板使用滑杆控制 35%–100% 的外层背景不透明度；文字、按钮和内容底板始终不透明，避免底层应用影响可读性。浮窗外区域保持清晰并可正常操作。
- 主界面“外观”提供跟随系统（默认）、浅色和深色；选择会保存并同步到浮窗。切换外观不会保存尚未完成的 API 表单、清除原文或重置正在进行的请求。
...
主界面使用 `theme/palettes.json` 中的语义颜色，原生浮窗使用对应的 `values/colors.xml` 与 `values-night/colors.xml`。状态栏和导航栏同步切换，菜单、占位符、候选译文、错误提示和分段计时均有明暗配色；系统强制反色关闭，避免双重反色。主题变化在现有视图上生效，不需要重启浮窗。

`npm run test:theme` 检查两套配色的普通文字（包括次要文字、占位符和按下状态）对比度不低于 4.5:1，交互边界不低于 3:1，并检查主界面与浮窗色值一致。依据 [WCAG 对比度说明](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum.html) 检查配色，不代表整款应用已通过完整的无障碍认证。
...
当前版本已验证 React Native 三语主界面、五语翻译、系统选中文本，以及支持透明度、拖拽缩放和请求分段计时的原生无障碍浮窗。浮窗及下拉菜单之外的区域保持可操作，不使用整屏背景模糊。尚未接入 Trime/Rime，也不会自动发送消息。
